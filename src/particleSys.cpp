...
particleSys::particleSys(vec3 source, vec3 partColor) {
  ...
  color = vec3(partColor.x, partColor.y, partColor.z);
}
...
void particleSys::drawMe(std::shared_ptr<Program> prog) {
  ...
  int c_pos = prog->getAttribute("vertCol");
  GLSL::enableVertexAttribArray(c_pos);
  //std::cout << "Any Gl errors1: " << glGetError() << std::endl;
  glBindBuffer(GL_ARRAY_BUFFER, colorbuffer);
  //std::cout << "Any Gl errors2: " << glGetError() << std::endl;
  CHECKED_GL_CALL(glVertexAttribPointer(1, 4, GL_FLOAT, GL_TRUE, 0, 0));
...
void particleSys::update(vec3 newStart) {
  ...
  for(auto particle : particles) {
        particle->update(t, h, g, newStart);
  }
  t += h;
